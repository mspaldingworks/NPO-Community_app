...
class ConversationPreview {
  ...
  ConversationPreview({required this.id, required this.username});
...
class ChatService extends ApiClient {
  ...
  Future<List<ChatMessage>> getConversation(int otherUserId) async {
    ...
    try {
      ...
      throw const FormatException(
        'Expected a list of messages for the conversation.',
      );
    } catch (e) {
      // Re-throw the exception with context
      throw Exception(
        'Failed to fetch conversation with user $otherUserId: $e',
      );
    }
  }
  ...
  Future<void> sendMessage({
    ...
    try {
      ...
      final isKnownSerializerBug =
          response.statusCode == 500 &&
          (body.contains("'dict' object has no attribute 'sender'") ||
              body.contains('no attribute') && body.contains('sender'));
  ...
  Future<List<ChatMessage>> getChannelMessages(
    ...
    try {
      ...
      final response = await read(urlPath: urlPath, jsonHeaders: authHeaders);
  ...
  Future<ChatMessage> sendChannelMessage({
    ...
    try {
      ...
      throw const FormatException(
        'Invalid response format when sending message',
      );
    } catch (e, stackTrace) {
      _logger.severe(
  ...
  Stream<WSMessage> connectToChatChannel(
    ...
    try {
      ...
      final wsUrl = AppConfig.current.webSocketUri(
        ApiEndpoints.wsConversation(channelId),
      );
      ...
      _subscriptions[channelId] = channel.stream.listen(
        ...
        onError: (error, stackTrace) {
          _logger.severe(
            'WebSocket error for channel $channelId',
            error,
            stackTrace,
          );
  ...
  void _scheduleReconnect(String channelId) {
    ...
    if (attempts >= _maxReconnectAttempts) {
      _logger.warning(
        'Max reconnection attempts reached for channel $channelId',
      );
      return;
    }
  ...
  void _disconnectChannel(String channelId, {bool notify = true}) {
    ...
    try {
      _channels[channelId]?.sink.close(status.goingAway, 'Client disconnected');
    } catch (e) {
